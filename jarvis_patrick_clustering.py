...
def computeSSE(data, labels, cluster_centers, cluster_map):
...
def adjusted_rand_index(true_labels, predicted_labels):
        ...
        for class_idx, class_label in enumerate(classes):
            for cluster_idx, cluster_label in enumerate(clusters):
                contingency_table[class_idx, cluster_idx] = np.sum((true_labels == class_label) & (predicted_labels == cluster_label))
...
def customDBSCAN(matrix, data, minPts):
    ...
    for i in range(n):
        ...
        if len(neighbors) < minPts:
            continue  # skip if inadequate data points
...
def jarvis_patrick(
    ...
    adjacency_matrix = adjacencyMatrix(data, k,t=2)
    computed_labels, cluster_centers, cluster_map = customDBSCAN(adjacency_matrix, data, smin)
    SSE = computeSSE(data, computed_labels, cluster_centers, cluster_map)
...
def jarvis_patrick_clustering():
    ...
    plt.title('Clusters with highest ARI')
    plt.xlabel('k')
    plt.ylabel('smin')
    plt.colorbar()
    plt.savefig('Q2 Clusters with highest ARI.png')
    plt.close()

    plt.scatter(k_values, smin_values, c=sses, cmap='viridis', s=25)
    plt.title('Clusters with lowest SSE')
    plt.xlabel('k')
    plt.ylabel('smin')
    plt.colorbar()
    plt.savefig('Q2 Clustering with lowest SSE.png')
    plt.close()
